...
$newObj .= "\t".'\'add-tags\': ['.(($_GET['add-tags'].length > 0) ? '\'' : '').implode($_GET['add-tags'],'\',\'').(($_GET['add-tags'].length > 0) ? '\'' : '').'],'."\n";
$newObj .= "\t".'\'primary-section\': \''.$_GET['primary-section'].'\','."\n";
$newObj .= "\t".'\'primary-tag\': \''.$_GET['primary-tag'].'\','."\n";
$newObj .= "\t".'\'features\': ['.(($_GET['features'].length > 0) ? '\'' : '').implode($_GET['features'],'\',\'').(($_GET['features'].length > 0) ? '\'' : '').'],'."\n";
$newObj .= "\t".'\'apple-news\': ['.(($_GET['apple-news'].length > 0) ? '\'' : '')..implode($_GET['apple-news'],'\',\'').(($_GET['apple-news'].length > 0) ? '\'' : '').'],'."\n";
